...
\textcolor{red}{profinite or ind-finite}
\end{proof}
...
\[\frac{\bH^1(K,T)}{\bH^1_{\FF}(K,T)}\to \prod_{\UU(\Pb)} \frac{\bH^1(K_\ku,T)}{\bH^1_{\FF}(K_\ku,T)}\]
...
\begin{proof}
Since $H^1_{\FF}(K,T)$ is finitely generated, then we only need to prove it contains no torsion. Consider the exact sequence
