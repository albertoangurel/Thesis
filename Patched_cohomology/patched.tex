...
\begin{definition}
An ultraprime is said to be a \emph{Kolyvagin ultraprime} is $\Frob_ku$ is conjugate to $\tau$ in $\Gal(K(T)_{p^\infty}/K)$.
\end{definition}
...
\[\bH^1_{\FF}(K,\bT)=\ker\Biggl(\bH^1(K,\bT) \to \prod_{\ku\in \UU(\Pb)} \frac{\bH^1(K_\ku,\bT)}{\bH^1_{\FF}(K_\ku,\bT)}\Biggr)\]
\end{remark}

The next technical proposition plays a central role in this theory.
...
\begin{definition}
    Let $\FF$ be a Selmer structure on $T$. The we can define a \emph{dual Selmer structure} $\FF^*$ on $T^*$ by defining the local condition $\bH^1(K_\ku,\bT^*)$ as the the annihilator of $\bH^1_{\FF}(K_\ku,\bT)$ under the local duality pairing in Proposition \ref{}\textcolor{red}{patched local duality}.
\end{definition}
