...
\[H^1_{\tr}(K_\ell,T):=H^1\Bigl(K(\ell)_\ell,T^{G_{K(\ell)_\ell}}\Bigr)\hookrightarrow H^1(K_\ell,T)\]
\end{definition}
...
\begin{proposition}(\cite[Lemma 3.5.3]{MazurRubin})
...
When the Selmer structure is cartesian, the Selmer group of some quotients of $A$ can be also identified with the torsion of the Selmer group.
...
\begin{proposition}
    ...
    \[H^1(K,T/I)\hookrightarrow H^1(K,T)[I]\]
...
Let $c_1,\ldots,c_s\in H^1(K,T)$ and let $c_1^*,\ldots,c_t^*\in H^1(K,T^*)$ be non-zero elements. If $s+t<p$, there is a Kolyvagin prime $\ell\in \PP$ such that $\loc_\ell(c_i)$ and $\loc_\ell(c_i^*)$ are all non-zero.
\label{prop:cheb}
\end{proposition}
...
is surjective. Then $H^1_{\FF(\ell)}(K,T^*)=H^1_{\FF}(K,T^*)$.
\label{lem:tr_res}
\end{lemma}

For the rest of this section, we assume $R$ is a principal ring. The next two lemmas show how we can make the Selmer group smaller by swapping the local condition at certain approppriate prime $\ell$. The situation when $\chi(\FF)\geq 1$ was done in \cite{MazurRubin} and it is technically since we can apply lemma \ref{lem:tr_res} directly.
...
When $\chi(\FF)=0$, we need to study quotients of $T$ in order to apply \ref{lem:tr_res} and, when recovering the information about the Selmer group at $t$, we only get partial information. The next lemma is the technical base for the main Theorems \textcolor{red}{cite} about the structure of Selmer group of core rank zero.
...
We study the propagated Selmer group to the quotient $T/\m^{e_1}$ and its dual $T^*[m^{e_1}]$. 

Applying Proposition \ref{prop:cheb} for some nonzero $c_1\in \m^{e_1-1} H^{1}_{\FF}(\Q,T/\m^{e_1})$ and some nonzero $c_1^*\in \m^{e_1-1} H^{1}_{\FF^*}(K,T[\m^{e_1}]^*)$, we can find infinitely many primes $\ell\in \PP$ such that the localisation maps \textcolor{red}{technicality Kolyvagin primes with quotients}
$$\begin{array}{cc}
\loc_\ell: H^1_{\FF}(\Q,T/\m^{e_1})\to H^1_\textrm{f}(\Q_\ell, T/\m^{e_1}),\ \ & \loc_\ell^*:H^1_{\FF^*}(\Q,T^*[\m^{e_1}])\to H^1_\f(\Q,T^*[\m^{e_1}])
\end{array}$$
are surjective. Since $\ell\in \PP$, both $H^1_\f(\Q_\ell, T/\m^{e_1})$ and $H^1_\f(\Q_\ell,T^*[e_1])$ are free $R/\m^{e_1}$-modules of rank one by Proposition \ref{prop:kol_primes}. \textcolor{red}{technicality with quotients}

By Proposition \ref{prop:selmer_torsion} and Lemma \ref{lem:tr_res},
\[H^1_{\FF^*(\ell)}(\Q,T^*)[\m^{e_1}]\cong H^1_{\FF^*(\ell)}(\Q,T^*[\m^{e_1}])=H^1_{\FF^*_\ell}(\Q,T^*[\m^{e_1}])\approx R/\m^{e_2}\times \cdots \times R/\m^{e_s}\]



Since $\chi(\FF^\ell)=1$ by Proposition \ref{prop:rank_modified}, then Proposition \ref{prop:core_rank_str} implies that
$$H^1_{\FF^*(\ell)}(\Q,T^*)\approx H^1_{\FF(\ell)}(\Q,T)\subset H^1_{\FF^\ell}(\Q,T)\approx R\oplus H^1_{\FF_\ell^*}(\Q,T^*)$$
...
When $\chi(\FF)=0$, we can improve Lemma \ref{lem:transverse_reduction_torsion} to obtain a result of the kind of Lemma \ref{lem:transverse_reduction_free} even when the Selmer group does not contain submodules isomorphic to $R$, but assuming some hypothesis about $T$ not being residually self-dual.
...
The Selmer groups defined in the previous section are the objects we want to study. We aim to give a description of the structure of this groups. In order to be precise, we introduce the concept of Fitting ideals.
...
\textcolor{red}{Fix the canonical homomorphism for the local cohomology / generator of $\GG_\ell$}
...
\subsection{Proof of Lemma \ref{lem:fitti_sub_fitt0trn}}

In order to prove Lemma \ref{lem:fitti_sub_fitt0trn}, we need to show, for any $n\in \NN_i$, the inclusion
...
Since all the prime divisors of $n$ are Kolyvagin primes, the last term is isomorphic to $R^{\nu(n)}$. The description of Fitting ideals over principal rings in Proposition \ref{prop:fitting_dvr} \textcolor{red}{(not fully exact, revise)} implies that 
...
Combining both inclusions, we can conclude the proof of Lemma \ref{lem:fitti_sub_fitt0trn}.
