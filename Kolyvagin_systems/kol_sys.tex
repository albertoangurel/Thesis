...
\begin{equation}
\Theta_i^{(0)}(\kappa,\FF)\subset\Fitt_i^R\Bigl(H^1_{\mathcal F}(\mathbb Q, T)\Bigr)
\label{eq:theta}
\end{equation}
 Moreover, if one of the following conditions is satisfied
\begin{enumerate}[(i)]
\item $i=\rank_R(H^1_{\FF}(\Q,T))=:r$
\item $\Theta_{i-1}^{(0)}(\kappa,\FF)\subsetneq \Fitt_{i-1}^R\left(H^1_{\mathcal F}(\mathbb Q, T)\right)$
...
\end{enumerate}
then we have the equality $\Theta_i^{(0)}(\kappa)=\Fitt_i^R\Bigl(H^1_{\mathcal F}(\mathbb Q, T)\Bigr)$.
\label{th:str_chi0}
\end{theorem}

\textcolor{red}{Divide proof in lemmas}
