...
\begin{proposition}
If $T$ satisfies Assumption \ref{ass:basic}, then $H^0(K,T/I)=0$ for every ideal $I$ of $R$
\label{prop:H0vanish}
\end{proposition}

\begin{proof}
By \ref{Tirred}, either $(T/\m T)^{G_K}=0$ or $T/\m T$ is the trivial representation. Since the latter does not satisfy \ref{Tcoh}, then the former holds. It implies that $(T/I)^{G_K}=0$.
\end{proof}
...
\[H^i_\f(K_\ell,T):=H^i(K_\ell^{\ur},T)=\ker\biggl(H^i(K_\ell,T)\to H^i(\II_\ell,T)\biggr)\]
...
\[H^i_\s(K_\ell,T)=H^i(K_\ell,T)\biggm/H^i_\f(K_\ell,T)\]
\end{definition}
...
\begin{definition}(Transverse cohomology)
Let $\ell\in \PP$. The \emph{transverse cohomology} sugbgroup is defined as
...
\begin{proposition}(\cite[Lemma 3.5.3]{MazurRubin}, \cite[Proposition 3.5]{BurnsSakamotoSano2})
    Under Assumptions \ref{ass:basic}, for every ideal $I$ of $R$, the inclusion $T^*[I]\hookrightarrow T^*$ induces an isomorphism
...
\label{eq:str_Fb}
\end{equation}
By Proposition \ref{prop:cheb_nd}, we can find infinitely many primes $\ell$ satisfying the following:
\begin{itemize}
