...
\[\bH^1_{(\FF^*){\kn}}(K,\bT^*)=0\]
while, for every ultraprime $\ku\mid \kn$, there is an equality
\[\bH^1_{(\FF^*)(\ku)}(K,T^*)=\bH^1_{(\FF^*)_\ku}(K,T^*)\]
...
\[\loc_{\ku^j}:\ \bH^1_{\FF}(K,\bT)\to \bH^1_\f(K_{\ku^j},\bT)\]
is surjective for each $j=1,\ldots,s$ and 
\[\bH^1_{(\FF^*)_\kn}(K,\bT^*)=0\]
The surjectivity of $\loc_{\ku_j}$ and the global duality exact sequence in Proposition \ref{prop:patched:global_duality} imply that, for every $j=1,\ldots,s$,
\[\bH^1_{(\FF^*)^\ku}(K,T^*)=\bH^1_{(\FF^*)}(K,T^*)\]
...
By proposition \ref{prop:iwasawa:bidual_map}, this exact sequence induces a map
...
We will assume the existence of a core vertex.
...
Before proving Theorem \ref{th:stark_core_projection}, we need to state some properties related to weak core vertices.

\begin{lemma}
Let $n\in \NN$ be a weak core vertex and let $m\in \NN$ be such that $n\mid m$. Then $m$ is also a weak core vertex.
\label{lem:weak_core_vertex_div}
\end{lemma}
...
The first and third terms of this exact sequence are free modules of ranks $r+\nu(n)$ and $\nu(m)-\nu(n)$. Hence, this exact sequence splits and thus $\bH^1_{\FF^m}(\Q,\bT)$ is free of rank $r+\nu(n)$.
\end{proof}


\begin{lemma}
Let $m\in \NN$ satisfy that $H^1_{\FF_m}(K,T^*)=0$. If Assumption \ref{ass:core_vertex} holds true, then $m$ is a core vertex and
\[H^1_{\FF^m}(K,T^*)\approx \Lambda^{r+\nu(m)}\]
\end{lemma}

\begin{proof}
Let $n\in \NN$ be the core vertex given by Assumption \ref{ass:core_vertex} $s\in \NN$ be a vertex divisible by $\textrm{lcm}(m,n)$. By Lemma \ref{lem:weak_core_vertex_div}, $s$ is also a core vertex and
\[\bH^1_{\FF^s}(K,T)\approx \Lambda^{r+\nu(s)}\]

By Proposition \ref{prop:global_duality}, there is an exact sequence
\[\xymatrix{0\ar[r] & \bH^1_{\FF^n}(\Q,\bT)\ar[r]&\bH^1_{\FF^{r}}(\Q,\bT)\ar[r] & \bigoplus_{\ell\mid \frac{s}{n}}\bH^1_s(\Q_\ell, \bT) \ar[r] &0}\]
The second and third modules of this short exact sequence are free $\Lambda$ modules of ranks $r+\nu(s)$ and $\nu(s)-\nu(n)$. Therefore, this sequence splits and 
\[\bH^1_{\FF^m}(K,T^*)\approx \Lambda^{r+\nu(m)}\]
\end{proof}



\begin{lemma}
For every $n\in \NN$, there exists a weak core vertex $n\in \NN$ such that $m\mid n$.
\label{lem:weak_core_vertex_existence}
\end{lemma}

\begin{proof}
A direct application of Proposition \ref{prop:iwasawa:cheb_dual_res} to the Selmer structure $(\FF^*)_m$ implies the existence of $n\in \NN$, divisible by $m$, such that 
\[\bH^1_{(\FF^*)_n}(K,T^*)=0\]
\end{proof}
...
\begin{proof}
By Assumption \ref{ass:core_vertex} and Lemma \ref{lem:weak_core_vertex_div}, there exists a core vertex $n$ such that $m\mid n$, which leads to the following exact sequence
