...
We want to prove an analogue with quotients by the elements divisible by the maximal ideal $\m$. This case is more complicated than Propositions \ref{prop:patched:quotients} and \ref{prop:iwasawa:quotient_injection} since the maximal ideal is not principal.
...
\begin{notation}
The next results construct suitable ultraprimes by building them from those obtained using Proposition \ref{prop:cheb}. In ocasion, we want to combine several results to guarantee the existence of ultraprimes satifying simultaneously more than one property. In order to to that, we say that the construct of an ultraprime involves $n$ choices if we are imposing that the localisations of $n$ cohomology classes do not vanish. By Proposition \ref{prop:cheb}, we can construct an ultraprime satisfying properties involving a total number of choices strictly smaller than $p$.
\end{notation}

\begin{proposition}
Let $\bT$ be a Galois representation over the Iwasawa algebra satisfying Assumptions \ref{ass:patched:basic}. Assume that $\bH^1_{\FF}(K,\bT)$ does not vanish. Then there is an ultraprime $\ku$ such that $\loc_\ku\neq 0$, whose construction involves two choices.
\end{proposition}
...
Since $\alpha\notin p\bH^1(K,\bT)$, Proposition \ref{prop:iwasawa:quotient_injection} implies that $\alpha$ projects to a non-zero element in $\bH^1(K,\bT/p\bT)$. By Proposition \ref{prop:selmer_profinite},
\[\bH^1_{\FF}(K,\bT/p\bT)=\varprojlim_m \bH^1_{\FF}(K,\bT_{1,m})\] 
...
\[\xymatrix{\bH_{\FF}^1(K,\bT_{n,m})\ar[r] & \bH^1_{\FF}(K,\bT_{1,m_0})\ar@{>->}[r] & \bH^1_{\FF}(K,\bT_{n,m})}\]
is the multiplication by $p^{n-1} X^{m-m_0}$. Since $\alpha_{1,m_0}$ is the image of $\alpha_{n,m}$ under the first map and the second map is injective by Proposition \ref{prop:H0vanish}, then $p^{n-1} X^{m-m_0} \alpha_{n,m}\neq 0$.
...
For every $a\in \N$, choose a sequence $\Bigl(\alpha_{a,a}^{(i)}\Bigr)$ representing $\alpha_{a,a}$. By the definition of ultraproduct and the closedness of ultrafilters under intersections, there is a set $S_a\in \UU$ such that, for every $i\in S$,
\[\begin{array}{cc}
p^{a-1} X^{a-m_0} \alpha_{a,a}^{(i)}\neq 0,\ p^{a-n_0} X^{a-1} \alpha_{a,a}^{(i)}\neq 0&
\end{array}\]
Denote $\iota_a:\ \N\to S_a$ the unique increasing bijection.

For every $a< \max\{n_0,m_0\}$, choose a random prime $\ell_a$. When $a\geq \max\{n_0,m_0\}$, we can choose, by using two choices of Proposition \ref{prop:cheb} a Kolyvagin prime $\ell_a\in \PP(T_{a,a})$ such that 
\[\begin{array}{cc}
\loc_{\ell_a}\Bigl(p^{a-1} X^{a-m_0} \alpha_{a,a}^{\iota(a)}\Bigr)\neq 0,\ & 
\loc_{\ell_a}\Bigl(p^{a-n_0} X^{a-1} \alpha_{a,a}^{\iota(a)}\Bigr)\neq 0
\end{array}\]

Construct the ultraprime $\ku$ represented by the sequence $(\ell_a)_{a\in \N}$. Consider the maps
\[\loc_\ku:\ \bH^1_{\FF}(K,T_{a,a})\to \bH^1_\f(K_\ku,T_{a,a})\]

Since $\ku$ is a Kolyvagin ultraprime, $\bH^1_\f(K_\ku,T_{a,a})$ is a free, cyclic $\Lambda_{a,a}$-module.

For every $b\geq a$, the construction implies that 
\[p^{a-1} X^{a-m_0} \loc_{\ell_b}\Bigl(\alpha_{b,b}^{(\iota(b))}\Bigr),p^{a-n_0} X^{a-1} \loc_{\ell_b}\Bigl(\alpha_{b,b}^{(\iota(b))}\Bigr)\notin H^1_\f(K_{\ell_b},\bT_{b,b})[p^{b-a}X^{b-a}]\]
Therefore,
\[p^{a-1} X^{a-m_0} \alpha_{b,b}^{(\iota(b))},p^{a-n_0} X^{a-1} \alpha_{b,b}^{(\iota(b))}\notin\ker\biggl(H^1_\f(K_{\ell_b},\bT_{b,b})\to H^1_\f(K_{\ell_b},\bT_{a,a})\biggr)\]

Then 

By Proposition \ref{prop:ultrafilter_exact} and taking limits, it means that $\Im(\loc_\ku)$ is not contained in either $p\bH^1_\f(K_\ku,T)$ or $X\bH^1_\f(K_\ku,T)$.

\textcolor{red}{redo, wrong}
